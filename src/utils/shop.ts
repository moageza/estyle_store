...
export const cartTotal = (cart: CartItem[]): number => cartSubtotal(cart) + shippingCost(cartSubtotal(cart));
...
export const trendingProducts = (): Product[] => PRODUCTS.filter((p) => p.isTrending);
